param(
  [string]$SonosRef = '87f409ab218a19a03cad630458258b291c365d8b',
...
$sourceRoot = Join-Path $WorkRoot 'sonoscli'
$outDir = Join-Path $WorkRoot 'out\sonos-android-arm64'
$outBinary = Join-Path $outDir 'sonos'
$assetBinary = Join-Path $repoRoot 'assets\openclaw\cli-core\bin\sonos'
...
if (Test-Path (Join-Path $sourceRoot '.git')) {
  ...
  git clone https://github.com/steipete/sonoscli.git $sourceRoot
}
git -C $sourceRoot checkout --detach $SonosRef
...
if ($bytes.Length -lt 1048576) {
  throw "Sonos payload is unexpectedly small: $($bytes.Length) bytes"
}
if ($bytes[0] -ne 0x7f -or $bytes[1] -ne 0x45 -or $bytes[2] -ne 0x4c -or $bytes[3] -ne 0x46) {
  throw 'Sonos payload is not an ELF executable'
}
if ($bytes[4] -ne 2 -or $bytes[5] -ne 1) {
  throw 'Sonos payload is not ELF64 little-endian'
}
$machine = [BitConverter]::ToUInt16($bytes, 18)
if ($machine -ne 183) {
  throw "Sonos payload is not AArch64. ELF machine=$machine"
}
