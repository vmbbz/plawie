param(
  [string]$EightctlRef = '2f2c73f0a529e9138707a237135fcaadfe56617e',
...
$sourceRoot = Join-Path $WorkRoot 'eightctl'
$outDir = Join-Path $WorkRoot 'out\eightctl-android-arm64'
$outBinary = Join-Path $outDir 'eightctl'
$assetBinary = Join-Path $repoRoot 'assets\openclaw\cli-core\bin\eightctl'
...
if (Test-Path (Join-Path $sourceRoot '.git')) {
  ...
  git clone https://github.com/steipete/eightctl.git $sourceRoot
}
git -C $sourceRoot checkout --detach $EightctlRef
...
if ($bytes.Length -lt 1048576) {
  throw "Eightctl payload is unexpectedly small: $($bytes.Length) bytes"
}
if ($bytes[0] -ne 0x7f -or $bytes[1] -ne 0x45 -or $bytes[2] -ne 0x4c -or $bytes[3] -ne 0x46) {
  throw 'Eightctl payload is not an ELF executable'
}
if ($bytes[4] -ne 2 -or $bytes[5] -ne 1) {
  throw 'Eightctl payload is not ELF64 little-endian'
}
$machine = [BitConverter]::ToUInt16($bytes, 18)
if ($machine -ne 183) {
  throw "Eightctl payload is not AArch64. ELF machine=$machine"
}
