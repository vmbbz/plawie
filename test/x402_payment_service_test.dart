...
void main() {
  final policy = X402PaymentPolicy(allowedHosts: <String>{'api.example.test'});
  ...
  test(
    'permits a resource-less top-up challenge only with an exact fallback',
    () {
  ...
  test('rejects wrong version, payee, timeout, and token-domain metadata', () {
    ...
    for (final changed in <Map<String, dynamic>>[
      ...
      _requirement(extra: <String, dynamic>{'name': '', 'version': '2'}),
      _requirement(extra: <String, dynamic>{'name': 'USD Coin', 'version': ''}),
    ]) {
      expect(
        () => X402PaymentChallenge.fromHeader(
          _encodedChallenge(accepts: <Map<String, dynamic>>[changed]),
