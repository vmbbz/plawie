...
class GatewayConnection {
  ...
  // 50 attempts Γëê 12+ minutes of exponential backoff.
  ...
  // Pending request completers ΓÇö keyed by request ID
  ...
  /// Shared-prefs key ΓÇö exposed so callers can purge the token on pairing recovery.
  ...
  void _onFrame(dynamic raw) {
    try {
      ...
      // ΓöÇΓöÇ Connect response (hello-ok) ΓöÇΓöÇ
      ...
      if (type == 'res' && frame['id'] == _connectRequestId) {
        ...
        if (ok) {
          ...
          // audit ΓåÆ pairing-required, even for already-approved devices.
      ...
      // ΓöÇΓöÇ Response to a pending RPC request ΓöÇΓöÇ
      ...
      if (type == 'event') {
        ...
        // Streaming chunks (agent, chat lifecycle) carry NO id field ΓÇö broadcast to
  ...
  // Pre-encoded ping frame ΓÇö allocated once, reused every 30s.
  ...
  Future<void> patchSessionMetadata(Map<String, dynamic> metadata) async {
    ...
    // Fire-and-forget via direct sink ΓÇö avoids registering a pending request controller
    // that would never be cleaned up (no listener ΓåÆ onCancel never fires), which would
  ...
  void _cleanup() {
    ...
    // Fire close() as a best-effort fire-and-forget ΓÇö this sends the WS close
