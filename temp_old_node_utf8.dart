...
class NodeService {
  ...
  Future<String?> _readGatewayToken() async {
    // 1. Primary: read directly from openclaw.json ΓÇö same authoritative source
  ...
  /// as a new device, allowing the normal NOT_PAIRED ΓåÆ auto-approve flow.
  Future<void> _handleNodePairingRequired() async {
    if (_pairingResolveAttempted) return;
    _pairingResolveAttempted = true;
    log('[NODE] Pairing required (1008) ΓÇö clearing stale device record...');
    try {
      ...
      log('[NODE] Device record cleared ΓÇö will re-pair on next connect');
      final prefs = PreferencesService();
      await prefs.init();
