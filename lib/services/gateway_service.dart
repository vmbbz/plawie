...
class GatewayService {
  ...
  Future<void> init() async {
    ...
    await getFilesDir();
    await _removeLegacyAutogeneratedHeartbeatFile();
    await _ensureMobileHeartbeatDefaultPersisted();
  ...
  Future<void> _removeLegacyAutogeneratedHeartbeatFile() async {
    try {
      ...
      for (final root in roots) {
        final heartbeat = File('$root/workspace/HEARTBEAT.md');
        if (!await heartbeat.exists()) continue;
        final contents = await heartbeat.readAsString();
        const legacyAutogeneratedContents = '''
# Plawie Heartbeat
...
HEARTBEAT_OK.
''';
        if (contents.trim() == legacyAutogeneratedContents.trim()) {
          if (await heartbeat
              .delete()
              .then((_) => true)
              .catchError((_) => false)) {
      ...
      if (removed > 0) {
        _addActivity(
          '[SYS] Removed $removed legacy auto-generated heartbeat file(s).',
        );
      }
    } catch (e) {
      debugPrint('[GatewayService] Legacy HEARTBEAT.md cleanup skipped: $e');
    }
  }

  bool _applyMobileHeartbeatDefault(Map<String, dynamic> config) {
    config['agents'] ??= <String, dynamic>{};
    final agents = config['agents'];
    if (agents is! Map) return false;
    agents['defaults'] ??= <String, dynamic>{};
    final defaults = agents['defaults'];
    if (defaults is! Map) return false;

    // Any valid heartbeat map is explicit user configuration and must win.
    // Only an absent/malformed value receives the data-safe mobile default.
    if (defaults['heartbeat'] is Map) return false;
    defaults['heartbeat'] = <String, dynamic>{'every': '0m'};
    return true;
  }

  Future<void> _ensureMobileHeartbeatDefaultPersisted() async {
    try {
      final config = await _readConfig();
      if (config.isEmpty || !_applyMobileHeartbeatDefault(config)) return;
      await _writeConfig(config);
      _addActivity(
        '[SYS] Disabled unconfigured autonomous model heartbeats on mobile.',
      );
    } catch (e) {
      debugPrint('[GatewayService] Mobile heartbeat policy skipped: $e');
    }
  }
  ...
  Future<void> _configureGateway({
    ...
    config['agents'] ??= {};
    config['agents']['defaults'] ??= {};
    _applyMobileHeartbeatDefault(config);
...
For OpenClaw phone, hardware, sensor, camera, canvas, weather, ClawHub metadata, meme image creation, location, screen, haptic, flashlight, or avatar gesture actions, call the OpenClaw nodes tool with "node": "$nodeHandle".
