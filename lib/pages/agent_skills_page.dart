...
class _AgentSkillsPageState extends State<AgentSkillsPage>
  ...
  final List<Map<String, dynamic>> _installedCatalog = [
    {
      ...
      'desc':
          'Full-body VRMA + limb layering (waves, bows, sits). 30+ mocap animations.',
      'installed': true,
  ...
  Future<void> _performClawHubSearch(String query) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("ClawHub connection issue. Try again later.")));
    } finally {
  ...
  void _showSkillProfile(Map<String, dynamic> skill) async {
    final profile =
        await _skills.getSkillProfile(skill['id'] ?? skill['slug'] ?? '');
              ...
              child: Row(
                children: [
                  ...
                  if (profile['verified'] == true)
                    const Chip(
                      label: Text('✓ Verified by ClawHub',
                          style: TextStyle(
                              color: Colors.black,
                              fontWeight: FontWeight.bold,
                              fontSize: 10)),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        profile['examples'] ??
                            "Plawie, help me with this skill!",
                        style: const TextStyle(
                            fontSize: 16,
                            color: Colors.cyanAccent,
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        await _skills
                            .installSkill(skill['slug'] ?? skill['id']);
                      ...
                      child: const Text("INSTALL / EQUIP",
                  ...
                  Expanded(
                    child: OutlinedButton(
                      onPressed: () async {
                        await _skills
                            .uninstallSkill(skill['slug'] ?? skill['id']);
                        if (!context.mounted) return;
                        Navigator.pop(context);
                        setState(() {});
                      },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Plawie's Skills & Tools",
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  ...
                  Text("Explore community skills on ClawHub",
                      style: TextStyle(
  ...
  Widget _buildSkillCard(
      Map<String, dynamic> skill, bool isInstalled, Color accentColor) {
    return Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(24),
        boxShadow: [
          BoxShadow(
              color: accentColor.withValues(alpha: isInstalled ? 0.2 : 0.05),
              blurRadius: 15,
                    ...
                    decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: accentColor.withValues(
                                alpha: isInstalled ? 0.5 : 0.1),
                            width: 2)),
                    child: Icon(skill['icon'] ?? Icons.auto_awesome,
                        size: 32,
                        color: isInstalled ? accentColor : Colors.grey),
                  ),
                  const SizedBox(height: 12),
                  Text(skill['name'],
                      style: const TextStyle(
  ...
  Widget _buildDiscoverListItem(Map<String, dynamic> skill) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(skill['name'] ?? skill['slug'] ?? 'Unknown',
            style: const TextStyle(
                color: Colors.white, fontWeight: FontWeight.bold)),
        subtitle: Text(
            skill['description'] ??
                skill['summary'] ??
                'ClawHub Community Skill',
            style: TextStyle(
        ...
        trailing: ElevatedButton(
          onPressed: () async {
            ...
            if (!mounted) return;
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text("Skill equipped! Plawie is ready."),
      ...
      onTap: () async {
        ...
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(
                  '${skill['name']} ${isInstalled ? 'removed' : 'equipped'}!'),
              backgroundColor:
                  isInstalled ? Colors.redAccent : Colors.greenAccent,
              behavior: SnackBarBehavior.floating));
        }
      },
      child: Container(
        ...
        child: Center(
            child: Text(isInstalled ? 'Uninstall' : 'Equip Skill',
                style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                    fontSize: 12))),
